...
- 2026-04-20 — Market Vision: árbol jerárquico Índice→Sector→ETF→Empresa en pestaña "Mercado". Añadidos ACWI/URTH/IEMG a etfs.json + etf_metadata (migración 003). `data/hierarchy.json` con 6 índices, 6 subsectores, 7 mappings ETF→subsector. `scripts/validate_hierarchy.py` con DFS cycle detection, integrado en CI (update-holdings workflow). Tab "ETFs & Sectores" renombrada a "Mercado".
